...
# File: Simulations for dynamic regressions with GARCH errors - Simulate y and fit AR(1) regression
...
lapply(1:nrow(params), function(p){ # iterate over parameters
  ...
  # Name the file
  filename <- paste0(path_results, dgp_name, "_fit_", params[p, "id"], ".RData")
  
  # Check if file exists
  if(!file.exists(filename)){
    
    # Simulate y and fit AR(1) model  
    fit_r <- replicate(R_mc, sim_ar2_fit_garch(N = N, I = I, a = a, phi1 = phi1, phi2 = phi2), simplify = "array")
