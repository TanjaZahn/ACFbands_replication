#' @title DGP: AR(1) with Chi^2 GARCH errors
#'
#' @description Generate data according to an AR(1) process with standardized Chi^2 GARCH errors.
#'
#'
#' @param N sample size
#' @param I length of burn-in period. Default is 50.
#' @param phi slope coefficient of AR(1) model.
#' @param a0 GARCH parameter. Set to 0.001.
#' @param a1 GARCH parameter. Set to 0.05.
#' @param b1 GARCH parameter. Set to 0.9.
#' @param k degrees of freedom of the chi^2 random variable. Set to 3.
#'
...
sim_ar1_y_garch_chi2 <- function(N , I = 50, phi, a0 = 0.001, a1 = 0.05, b1 = 0.9, k = 3){
  
  # Draw chi2 random variable
  rv <-  rchisq(n = N + I, df = k)
