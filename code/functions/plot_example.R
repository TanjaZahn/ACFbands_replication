...
plot_example <- function(df, yvar, colorvar, ylabel, hline = NULL){
  ...
  p <- ggplot(df, aes(x = H, y = !!yvar)) +
    geom_line(aes(color = !!colorvar, linetype = !!colorvar)) +
    # geom_point(aes(shape=!!colorvar),  size = 1) +
    scale_color_manual(labels = mylabels,  values = mycolors) +
    scale_linetype_manual(labels = mylabels, values = mylines) + 
    # scale_shape_manual(labels = mylabels, values = myshapes) + 
    labs(x = "H", y = ylabel) +
    mytheme
