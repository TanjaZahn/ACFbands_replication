...
# File: Conducts the macroeconomic application of the paper using the full sample, including:
#           # construction of confidence bands for inflation itself;
#           # estimation of static and dynamic Phillips curve (in differences and in levels);
...
df_conf_classic <- bind_rows(lapply(conf_types, function(type){
  
  estimate <- acf_confbands(y = df_fred$inf, H = H, covar = "classical", 
                            L = floor(sqrt(N)), alpha = alpha, type = type, plot = FALSE)
...
df_conf_robust <- bind_rows(lapply(conf_types[3], function(type){
  
  estimate <- acf_confbands(y = df_fred$inf, H = H, L = floor((length(N-H)^(1/3))), 
                            alpha = alpha, type = type, plot = FALSE, covar = "robust")
  df <- data.frame(rho_hat = estimate$rho_hat , estimate$conf_band, h = 1:H)
  
}), .id = "type") %>% 
  mutate(type = str_replace_all(type, label_conf_robust)) # use labels

# Combine data frames
df_conf <- bind_rows(df_conf_classic, df_conf_robust)

# Plot
p_inf_conf <- make_plot_color(df = df_conf, segment = FALSE, line_vec = mylines, color_vec = mycolors) +
  theme(legend.position=c(0.2,0.15))
# ggsave(paste0(path_graphics, "inflation_conf_bands.pdf"), width = 20, height = 12, units = "cm") 
...
df_sig_classic <- bind_rows(lapply(sig_types[1:2], function(type){
  ...
  estimate <- acf_sigbands(y = res, H = H, alpha = alpha, type = type, plot = FALSE, covar = "iid", adjust_N = TRUE)
...
df_sig_robust <- bind_rows(lapply(sig_types[3], function(type){
  ...
  estimate <- acf_sigbands(y = res, H = H, alpha = alpha, type = type, 
                           covar = "MDS", plot = FALSE, adjust_N = TRUE)
  df <- data.frame(rho_hat = estimate$rho_hat , estimate$sig_band, h = 1:H)
...
# Plot
p_phil_sig <- make_plot_color(df = df_sig, segment = TRUE,  line_vec = mylines, color_vec = mycolors) +
  theme(legend.position=c(0.2,0.15))
# ggsave(paste0(path_graphics, "phillips_sig_bands.pdf"), width = 20, height = 12, units = "cm")

p_phil_sig | p_inf_conf
ggsave(paste0(path_graphics, "phillips_and_inflation.pdf"), width = 35, height = 15, units = "cm") 
