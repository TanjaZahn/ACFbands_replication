...
# File: Simulations for time series with GARCH errors - Setup
...
sig_types <- c("sup-t", "pointwise")
names(sig_types) <- sig_types

# Vector of bandtypes for confidence bands
conf_types <- c("sup-t", "bonferroni", "pointwise")
names(conf_types) <- conf_types # c("sup_t", "bonferroni", "pointwise")

# Vector of tests
# test_vec <- c("boxpierce", "ljungbox", "shao", "dalla")
test_vec <- c("boxpierce", "ljungbox", "shao", "bp-robust")
names(test_vec) <- test_vec


# Labels: Classic Bands
label_conf <- c("sup-t" = "Classical sup-t CB",
                "bonferroni" = "Classical Bonf. CB",
                "pointwise" = "Classical pointw. CB")

label_sig <- c("sup-t" = "Classical sup-t SB",
               "pointwise" = "Classical pointw. SB")

label_tests <- c("boxpierce" = "Box-Pierce", 
                 "ljungbox" = "Ljung-Box",
                 "shao" = "Shao", 
                 "bp-robust" = "Robust Box-Pierce")
