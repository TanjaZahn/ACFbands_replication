...
# File: Simulations for time series with GARCH errors - Construct classical significance bands and run alternative hypotheses tests.

#Set Up 
source("code/sim_ts_garch/sim_ts_garch_01_setup.R")

# Simultaneous Significance Bands (classic) -------------------------------------------------
...
lapply(1:nrow(params), function(p){ # iterate over parameters
  ...
  # Print the iteration step
  print(paste0(format(Sys.time(), "%H:%M")," : test = ", test, ";  p = ", p))
  ...
  # Filename
  filename <- paste0(path_results, dgp_name, params[p, "id"], "_sig_", test, ".RData")
