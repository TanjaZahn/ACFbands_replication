...
# File: Simulations for time series with GARCH errors - Construct classical confidence bands.
    ...
    lapply(1:nrow(params), function(p){ # iterate over parameters
      ...
      if(!file.exists(filename)){
        ...
        estimates_r <- lapply(1:R_mc, function(r){
          ...
          out <- try(acf_confbands(y = y_r[ , r], H = params[p, "H"], type = band_type, covar = "classical", 
                                   B_hat = B_hat_r[[r]],  L =  params[p, bw], alpha = alpha, plot = FALSE))
