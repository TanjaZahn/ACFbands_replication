...
# File: Simulations for time series with Chi Square GARCH errors - Make tables.
...
# # Robust significance bands ----------------------------------------------------
# 
# df_rej <- df_sigbands_robust %>% select(type, rej, H, phi, N) %>% 
#   arrange(phi, N)
# 
# # Table for frequency of rejections
# make_table(df = df_rej,
#            x = "rej",
#            rnames = unique(df_rej$type), # sig_types,
#            panel_names = panel_names1,
#            filename =  paste0(path_graphics, "/tab_", dgp_name, "_sig_rej_MDS.tex"))
...
make_table(df = df_cover_sel %>% dplyr::select(type, cover, H, phi, N),
           x = "cover",
           rnames = unique(df_cover_sel$type), # c("Sup-t", "Bonf.", "Pointw."),
