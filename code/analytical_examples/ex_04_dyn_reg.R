...
# Into data frame
sigband_simul <- data.frame(lb, ub, width, h = 1:H, type = rep("Corrected classical sup-t SB", H))
...
sigband_naive_simul <- data.frame(lb, ub, width,  h = 1:H, type = rep("Naive classical sup-t SB", H))
...
ggplot(df, aes(x = h)) +
  ...
  geom_line(aes(y = lb, color = type, linetype = type)) +
  geom_line(aes(y = ub, color = type, linetype = type)) +
  ...
  theme(legend.position = c(0.2, 0.14), legend.margin=margin(c(0)))
